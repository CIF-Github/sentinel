...
class DashConfig():
    ...
    def slurp_config_file(self, filename):
        # read cif.conf config but skip commented lines
        f = io.open(filename)
        lines = []
        for line in f:
            if re.match('^\s*#', line):
        ...
        # data is cif.conf without commented lines
        data = ''.join(lines)
    ...
    def get_rpc_creds(self, data, network='mainnet'):
        ...
        default_port = 43198 if (network == 'mainnet') else 23798

        # use default port for network if not specified in cif.conf
        if not ('port' in creds):
            creds[u'port'] = default_port

        # convert to an int if taken from cif.conf
